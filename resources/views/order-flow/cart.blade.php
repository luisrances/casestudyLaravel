<x-layout>
    <main class="container mx-auto px-4 py-3 pt-0 flex-grow h-[140vh] lg:h-auto">
        <div class="ml-4 text-[18px]">
            ...
            {{-- <a class="text-lg font-semibold mb-3 px-4 pb-1 mr-[30px]"
                href="{{ route('purchase_history.user') }}">Purchase History</a> --}}
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8 mt-5 lg:max-h-[500px] max-h-[800px]">
                                        ...
                                        @foreach ($products as $product)
                                            @if (Auth::user()->id == $cart->account_id && $cart->product_id == $product->id)
                                                <tr class="bg-white border-b hover:bg-gray-50">
                                                    <td class="px-5 py-4">
                                                        <input type="checkbox" name="selected_cart_items[]" value="{{ $cart->id }}"
                                                            class="w-4 h-4 text-blue-600 bg-white border border-gray-500 rounded hover:bg-gray-100 focus:ring-blue-500">
                                                    </td>
                                                    <td class="py-4 ml-0 w-[110px] h-24">
                                                        @if ($cart->product_id == $product->id && $product->image_path)
                                                            <img src="{{ asset('storage/' . $product->image_path) }}" alt="{{ $product->name }}"
                                                                class="w-16 md:w-32 max-w-full max-h-full object-contain  inline-flex justify-center">
                                                        @else
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor"
                                                                class="bi bi-bag w-16 md:w-32 max-w-full max-h-full  inline-flex justify-center"
                                                                viewBox="0 0 16 16">
                                                                <path
                                                    ...
                                                    <td class="px-6 py-4 text-black">
                                                        <div class="flex justify-center items-center">
                                                            <button type="button"
                                                                class="decrement-btn inline-flex items-center justify-center h-6 w-6 p-1 text-sm font-medium text-gray-500 bg-white border border-gray-500 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-2 focus:ring-gray-100"
                                                            ...
                                                            <input type="number" name="quantity" value="{{ $cart->quantity }}" min="1"
                                                                ...
                                                                data-cart-id="{{ $cart->id }}" data-max-stock="{{ $product->stock }}" />
                                                            <button type="button"
                                                                class="increment-btn inline-flex items-center justify-center h-6 w-6 p-1 text-sm font-medium text-gray-500 bg-white border border-gray-500 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-2 focus:ring-gray-100"
                                                                data-cart-id="{{ $cart->id }}">
                                                                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                    viewBox="0 0 18 18">
                                                                    <path stroke="currentColor" stroke-linecap="round"
            ...
            <section
                class="w-full lg:w-[400px] bg-white p-6 rounded-lg shadow-md static lg:sticky lg:top-[13%] z-0 h-fit [box-shadow:0_0_10px_rgba(0,0,0,0.2)]">
                ...
                <button id="checkout-btn"
                    class="mt-6 w-full bg-blue-500 text-white py-3 rounded-md text-lg font-semibold hover:bg-blue-400 transition duration-200">
                    Proceed to Checkout
                </button>
            </section>
        </div>
    ...
    <div id="stock-alert-modal"
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">Stock Limit Reached</h3>
                <div class="mt-2 px-7 py-3">
                    <p class="text-sm text-gray-500" id="stock-message"></p>
                </div>
                <div class="items-center px-4 py-3">
                    <button id="stock-alert-close"
    ...
    <script>
        function showStockAlert(maxStock) {
            document.getElementById('stock-message').textContent = `Maximum available stock is ${maxStock}`;
            document.getElementById('stock-alert-modal').classList.remove('hidden');
        }

        // Add event listener for the close button
        document.getElementById('stock-alert-close').addEventListener('click', function () {
        ...
        document.querySelectorAll('.cart-qty-input').forEach(input => {
            ...
            input.addEventListener('change', function () {
                ...
                if (parseInt(this.value) > maxStock) {
                    this.value = maxStock;
                    quantity = maxStock;
                    showStockAlert(maxStock);  // Replace alert with this
                }
