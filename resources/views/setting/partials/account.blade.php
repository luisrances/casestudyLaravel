<div id="account" class="section px-4">
    ...
    <form action="{{ route('account.update', $account) }}" method="POST" enctype="multipart/form-data"
        onsubmit="handleAccountSetting(event)">
        @csrf @method('PATCH')

        <div class="flex flex-col md:flex-row items-center gap-6 mb-4 border-b pb-4">
            <div
                class="w-24 h-24 bg-gray-200 rounded-full flex items-center justify-center overflow-hidden shadow-inner">
                @if (!empty($account->image))
                    <img src="{{ asset('storage/' . $account->image) }}" alt="Profile Photo"
            ...
            <div class="flex flex-col items-center md:items-start">
                ...
                <div class="flex gap-4">
                    <a href="#" class="text-blue-600 font-medium hover:underline text-sm"
                        onclick="document.getElementById('image').click(); return false;">Upload new photo</a>
                    <input type="file" class="hidden block text-sm font-medium text-gray-700 mb-1" id="image"
                        name="image" accept="image/*" onchange="previewImage(event)">
                    <a href="#" class="text-red-500 font-medium hover:underline text-sm"
                        onclick="removePhoto(event)">Remove photo</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 mb-4">
            <div>
                ...
                <input type="text" id="first_name" name="first_name"
                    value="{{ old('first_name', $account->first_name ?? '') }}" placeholder="Enter your first name"
            ...
            <div>
                ...
                <input type="text" id="last_name" name="last_name"
                    value="{{ old('last_name', $account->last_name ?? '') }}" placeholder="Enter your last name"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3"
                    readonly>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-9"> {{-- Added mb-10 for spacing before buttons
            ...
            <div>
                ...
                <input type="password" id="password" name="password" value="" placeholder="********"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 bg-gray-200 cursor-not-allowed"
                    disabled readonly>
            </div>
        </div>
